// Package migrate runs the guest-pass plugin's embedded SQL migrations against
// the caller-provided database. The operator pre-creates the `guest_pass`
// schema and the plugin role; migrations apply DDL inside that schema.
package migrate
...
// dsn must include search_path=guest_pass so unqualified DDL targets the
